"""Three concurrent clients increment one map key without locking.

Expected: final value is often lower than expected due to lost updates.
"""
...
def worker(worker_id: int) -> None:
    client = create_client(f"no-lock-worker-{worker_id}")
    try:
...
def main() -> None:
    initializer = create_client("no-lock-initializer")
    try:
        ...
        print(f"Lost updates:         {expected - final_value}")
    finally:
